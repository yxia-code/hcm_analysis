...
### Run ANCOM
res = ANCOM(feature_table = feature_table, meta_data = meta_data, struc_zero = NULL, main_var = main_var, p_adj_method = 'BH', alpha=p_cutoff, adj_formula=adj_formula, rand_formula='~1|short_PID')

### output 
write_csv(res$out, paste(out_path, 'ancom_results.csv', sep='/'))
pdf(paste(out_path, 'ancom_plot.pdf', sep='/'))
